/**
 * Dashboard Page — Smart Driver Assistant
...
import { getCurrentTrip, quickWalkin, sendDepartureNotification, cancelPassenger, clearNoShow, abandonTrip } from '../services/api';
...
export default function Dashboard({
    ...
    const handleClearNoShow = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Remove no-show passengers from list
    ...
    const usedSeats = (trip.total_seats || 13) - trip.available_seats;
    const progress = (usedSeats / (trip.total_seats || 13)) * 100;
        ...
        <div className="dashboard-container">
            ...
            <div className="seat-section">
                ...
                <p className="seats-remaining">
                    {isFull ? '🚫 เต็มแล้ว!' : `ว่างอีก ${trip.available_seats} ที่นั่ง`}
                </p>
            </div>
            ...
            {/* Action Buttons — context-aware highlights */}
                            ...
                            <div className="passenger-group">
                                ...
                                <div className="passenger-list">
                                    {checkedInPassengers.map((p, idx) => {
                                        const isWalkin = (p.type || p.queue_type) === 'walkin';
                                        const ticketId = p._id?.slice(-4)?.toUpperCase() || '----';
                                        return (
                                            <div key={p._id} className="passenger-item checked_in">
                                                <span className="passenger-num">#{idx + 1}</span>
                                                <span className="passenger-name">
                                                    {isWalkin ? `Walk-in — ตั๋ว #${ticketId}` : p.passenger_name}
                                                </span>
                                                <span className={`passenger-type ${p.type || p.queue_type}`}>
                ...
                <div className="cancel-modal-overlay" onClick={() => setCancelTarget(null)}>
                    <div className="cancel-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <p>
                            ...
                            <strong> #{cancelTarget._id?.slice(-4)?.toUpperCase()}</strong> ใช่หรือไม่?
                        </p>
